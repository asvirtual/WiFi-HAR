...
if __name__ == "__main__":
    ...
    # choice of the best temperature for the choice of the important channel to consider:
    checkpoint = torch.load(checkpoint_path)
    model.load_state_dict(checkpoint['model_state_dict'])
    model.eval()
    all_logits, all_labels = [], []
    with torch.no_grad():
        for bx, by in valid_dataloader:
            bx = bx.view(-1, 1, 340, 100).to(device)
            all_logits.append(model(bx).view(-1, 4, 8))
            all_labels.append(by)

    logits = torch.cat(all_logits, dim=0)
    labels = torch.cat(all_labels, dim=0).to(device)

    # 2. Grid Search on the temperature
    probs = torch.softmax(logits, dim=-1)
    entropy = -torch.sum(probs * torch.log(probs + 1e-8), dim=-1)

    best_T, best_acc = 1.0, 0.0
    for T in [0.1, 0.25, 0.5, 0.75, 1.0, 1.5, 2.0]:
        w = torch.softmax(-entropy / T, dim=1).unsqueeze(-1)
        acc = ((probs * w).sum(dim=1).argmax(dim=1) == labels).float().mean().item()
        if acc > best_acc:
            best_acc, best_T = acc, T

    checkpoint['temperature'] = best_T
    torch.save(checkpoint, checkpoint_path)
