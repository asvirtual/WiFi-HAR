...
class AntennaAttentionFusion(torch.nn.Module):
    def __init__(self, num_classes=8):
        super().__init__()
        self.attention = Sequential(
            Linear(num_classes, 32),
            Tanh(),
            Linear(32, 1)
        )

    def forward(self, y_pred_4antennas):
        # y_pred_4antennas: (Batch, 4, Num_Classes)
        scores = self.attention(y_pred_4antennas) # (Batch, 4, 1)
        weights = torch.softmax(scores, dim=1) # (Batch, 4, 1)
        y_fused = torch.sum(weights * y_pred_4antennas, dim=1) # (Batch, Num_Classes)
...
if __name__ == "__main__":
    ...
    opt = AdamW(list(model.parameters()) + list(fusion_layer.parameters()), lr=5e-5, weight_decay=5e-4)
    ...
    for epoch in range(epochs):
        ...
        for batch_x, batch_y in train_iterator:
            ...
            y_pred_fused, _ = fusion_layer(logits)
        ...
        with torch.no_grad():
            ...
            for batch_x, batch_y in val_iterator:
                ...
                batch_loss = ce_loss_fn(y_pred_fused, batch_y)
