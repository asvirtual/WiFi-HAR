...
if __name__ == "__main__":
    ...
    best_val = np.inf
    best_val_acc = -np.inf
    checkpoint_path = "./models/contrastive2_model.pt"
    window_size = 5
    val_loss_window = []
    val_acc_window = []
    ...
    for epoch in range(epochs):
        ...
        for batch_x, batch_y in train_iterator:
            ...
            fused_logits = torch.sum(logits * weights, dim=1)  # fuse the logits directly
            loss_ce = ce_loss_fn(fused_logits, batch_y)
        ...
        with torch.no_grad():
            ...
            for batch_x, batch_y in val_iterator:
                ...
                weights = torch.softmax(-entropy / temperature, dim=1).unsqueeze(-1)  # (size, 4, 1)
                fused_logits = torch.sum(logits * weights, dim=1)  # fuse the logits directly
                batch_loss = ce_loss_fn(fused_logits, batch_y)
    ...
    history_path = "plot_data/training_history_contrastive2.json"
